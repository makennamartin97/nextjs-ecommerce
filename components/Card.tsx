...
//creating prop type for component, injecting stripe price obj
type CardProps = {
    price:Stripe.Price
}
...
const Card:FunctionComponent<CardProps> = ({price}) => {
    ...
    const addToCart = (p: Stripe.Price) => {
        console.log('add to cart add',add,'p',p)
        if (add) {
            add(p)
        }
    }
    return (
        ...
        <div className="mt-6">
            <button
                ...
                className="btn relative w-full flex bg-gray-200 border border-transparent rounded-md py-2 px-8 items-center justify-center text-sm font-medium text-gray-900 hover:bg-gray-200"
            >
